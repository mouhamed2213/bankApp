    ...
    <div class="bg-gray-50 min-h-screen">
        ...
        <div class="py-12">
            <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
                ...
                {{-- La carte principale pour le formulaire de dépôt --}}
                <div class="bg-white shadow-lg rounded-xl overflow-hidden">
                    <form method="POST" action="{{ route('transaction.deposit') }}">
                        @csrf
                        <div class="p-6 sm:p-8">
                            <h3 class="text-lg font-semibold text-gray-800 mb-1">Détails du dépôt</h3>
                            <p class="text-sm text-gray-500 mb-6">Choisissez le compte à créditer et le montant.</p>

                            <div class="space-y-6">

                                <!-- Sélection du compte à créditer -->
                                <div>
                                    <label for="choosedAccount" class="block text-sm font-medium text-gray-700">Dans quel compte déposer ?</label>
                                    <select id="choosedAccount" name="choosedAccount" required class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm">
                                        ...
                                        <option value="{{ $account->numero_compte }}">
                                            Compte {{ ucfirst($account->type_de_compte) }} ({{ $account->numero_compte }})
                                        </option>
                                        @endif
                                        @endforeach
                                    </select>
                                    {{-- Message d'erreur pour la sélection du compte --}}
                                ...
                                <!-- Montant à déposer -->
                                <div>
                                    <label for="amount" class="block text-sm font-medium text-gray-700">Montant à déposer (FCFA)</label>
